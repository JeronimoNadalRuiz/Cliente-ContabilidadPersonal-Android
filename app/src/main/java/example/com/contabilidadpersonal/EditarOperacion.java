...
public class EditarOperacion extends AppCompatActivity implements View.OnClickListener, TextWatcher {
    String tipoValue, conceptoValue, cantidadValue, usuarioValue, predefinidoValue, id;
    EditText cantidad;
    ...
    private void guardarOperacion() {
        if(validacion()){
            url =((MyApplication) this.getApplication()).getIp() + "/operacion/"+id;
            RequestQueue colaPeticiones = Volley.newRequestQueue(this);
            MostrarManager mostrarManager = new MostrarManager();
            StringRequest peticion =
                    new StringRequest(Request.Method.PUT,
                            url,
                            mostrarManager,
                            mostrarManager){
                        @Override
    ...
    private void getOperacion(String id) {
        url = ((MyApplication) this.getApplication()).getIp() + "/operacion/id/" + id;
        RequestQueue colaPeticiones = Volley.newRequestQueue(this);
        MostrarManager3 mostrarManager3 = new MostrarManager3();
        StringRequest peticion =
                new StringRequest(Request.Method.GET,
                        url,
                        mostrarManager3,
    ...
    class MostrarManager3 implements Response.Listener<String>, Response.ErrorListener {
        @Override
        public void onErrorResponse(VolleyError error) {
        ...
        public void onResponse(String response) {
            try {
                ...
                JSONObject operaciones = cast.getJSONObject(0);
                cantidad.setText(operaciones.getString("cantidad"));
                concepto.setText(operaciones.getString("concepto"));
